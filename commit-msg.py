...
def commit_message(diff, model):
    ...
    tcount = token_count(prompt + diff, model)
    if tcount <= max_token_count[model]:
        return gpt(prompt + diff)

    result = []
    summaries = summarize(diff, model)
    result.extend(summaries)
    summary_string = "\n\n".join(summaries)
    while True:
        if token_count(prompt + summary_string, model) <= max_token_count[model]:
            break
        # Summarize the summary
        summaries = summarize(summary_string, model)
        result = summaries + ["## More Detail"] + result
        summary_string = "\n\n".join(summaries)
    
    result.insert(0, gpt(prompt + summary_string, model))
    return "\n\n".join(result)
...
def summarize(text, model="gpt-3.5-turbo",
    ...
    chunk = [parts[0]]
    chunk_tcount = token_count(parts[0], model)
    for part in parts[1:]:
        part_tcount = token_count(part, model)
        #print(f">>> {splitre[0]!r}",
        #      chunk_tcount,
        #      len(chunk),
        #      part_tcount,
        ...
        if chunk_tcount + part_tcount >= max_token_count[model]:
            text = "".join(chunk)
            chunk = []
            if token_count(text, model) > max_token_count[model]:
                # Need to split using a different regex
                summaries.extend(summarize(text, splitre=splitre[1:]))
            else:
                summaries.append(gpt(prompt + text))
            chunk_tcount = sum(token_count(c, model) for c in chunk)
        chunk.append(part)
...
def main():
    ...
    print(commit_message(diff, model))
    print(f"({model})")
